...
def win_screen(player, rounds):
    ...
    def display_text():
        win = "Congratulations " + player + " You Won in " + \
            str(rounds) + " rounds, Press SPACE to Play again."
...
#-----------Big Ships to be displayed on the side to show whose turn it is---------#
    ...
    s1, s2, s3 = pygame.image.load(
        'ship1-15.png'), pygame.image.load('ship2-15.png'), pygame.image.load('ship3-15.png')
    s4, s5, s6 = pygame.image.load(
        'ship4-15.png'), pygame.image.load('ship5-15.png'), pygame.image.load('ship6-15.png')
...
#-----Some Queue functions to be used-----------#
    ...
    one, two, three = pygame.image.load('dice1.png'), pygame.image.load(
        'dice2.png'), pygame.image.load('dice3.png')
    four, five, six = pygame.image.load('dice4.png'), pygame.image.load(
        'dice5.png'), pygame.image.load('dice6.png')

    #----Loading asteroids and setting their cordinates------------#
    asty1, asty2 = pygame.image.load(
        "asty1.png"), pygame.image.load("asty2.png")
    ...
    if size == 10:
        snakeImg, snakeTail = pygame.image.load(
            'snekPortal10.png'), pygame.image.load('snekPortal10_X.png')
        ladderImg, ladderHead = pygame.image.load(
            'portal10.png'), pygame.image.load('portal10_X.png')
    if size == 15:
        snakeImg, snakeTail = pygame.image.load(
            'snekPortal15.png'), pygame.image.load('snekPortal15_X.png')
        ladderImg, ladderHead = pygame.image.load(
            'portal15.png'), pygame.image.load('portal15_X.png')
...
#---------Convert a (x,y) cordinate into a number location-----------#
    ...
    key10, key15 = pygame.image.load(
        'key10.png'), pygame.image.load('key15.png')
    ...
    def board(x, y, number):
        if size == 10:
            radius, font_size = 60, 32
        if size == 15:
            radius, font_size = 40, 15
        A = radius//2
    ...
    def whose_turn(x):
        ...
        if x:  # --------If x is True------------#
            ...
            display_txt(surfaceToName(next_ship) +
                        ", Roll the Dice", text_x, text_y, 31)

    #-----Initially the dice is zero---------#
    dice = 0

    #-------All the players----------#
    heroes = ["Daedalus", "Explorer", "Excalibur",
              "Intrepid", "Odyssey", "Pleiades"]
    # --------This dictionary will story the location (x,y) of each player----------------#
    players = {}
    for i in range(num_players):
        # -------The initial positions of each player--------#
        players[heroes[i]] = (0, 600)
    # --------The players that are playing-------------#
    spaceships = heroes[:num_players]
    ...
    while running:  # ------ Keep on running the game unless player presses quit--------#
        ...
        while moving:
            ...
            snake = False
            # -------Whose turn is it?---------#
            # -------Getting the player in the front of the queue-------#
            current_player = deQueue(spaceships)
            ship = current_player  # ---------The current player----------#
            # -------Adding that player back to the queue--------#
            enQueue(spaceships, current_player)
            counter += 1  # -------After each turn the counter increases by 1--------------#
            # ------Cordinates of that player------#
            # ---The x-y location of the current-player
            xy_position = players[current_player]
            # ------The x cordinate of the current player-------#
            XX = xy_position[0]
            # -----The y cordinate of the current player---------#
            YY = xy_position[1]
        ...
        if all_players[0]:
            ship1_x, ship1_y = players["Daedalus"][0], players["Daedalus"][1]
        #----updated XY-cordinate of ship2--------#
        if all_players[1]:
            ship2_x, ship2_y = players["Explorer"][0], players["Explorer"][1]
        # ----updated XY-cordinate of ship3--------#
        if all_players[2]:
            ship3_x, ship3_y = players["Excalibur"][0], players["Excalibur"][1]
        # ----updated XY-cordinate of ship4--------#
        if all_players[3]:
            ship4_x, ship4_y = players["Intrepid"][0], players["Intrepid"][1]
        # ----updated XY-cordinate of ship5--------#
        if all_players[4]:
            ship5_x, ship5_y = players["Odyssey"][0], players["Odyssey"][1]
        # ----updated XY-cordinate of ship6--------#
        if all_players[5]:
            ship6_x, ship6_y = players["Pleiades"][0], players["Pleiades"][1]

        # -------Display a ship that is next in the queue-----------#
        whose_turn(ship)
...
def start():  # ------The Welcome Display-------#
    ...
    def display_txt(n, x, y, font_size, b=True):
        if b:
            msg = "Number of Players: Enter a number ranging from 1 to 6 in your keyBoard."
        else:
            msg = "You want "+str(n)+" players. Select board: A or B"
        font = pygame.font.Font('freesansbold.ttf', font_size)
        text = font.render(msg, False, (255, 255, 255))
        textRect = text.get_rect()
        textRect.center = (x, y)
    ...
    while intro:
        ...
        screen.blit(background, (0, 0))
        if not begin:
            display_txt(num_players, 600, 640, 32)
        else:
            display_txt(num_players, 600, 640, 32, False)
